        ...
        public static void Smites(Obj_AI_Base target)
        {
            if (MapSupported)
            {
                if (target != null && target.IsValidTarget() && IsReady)
                {
                    ...
                    {
                        if (SmiteDamageMinions >= (target.Health + target.AttackShield + target.MagicShield) && Vector3.Distance(ObjectManager.Player.ServerPosition, target.ServerPosition) <= smiterange)
                        {
                            ObjectManager.Player.Spellbook.CastSpell(SmiteSlot, target);
                        }
                    }
